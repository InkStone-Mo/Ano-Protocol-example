    ...
    while (1) {
        ...
        while (1) {
            ...
            // Data received
                ...
                rx_buffer[len] = 0;
                Ano_ProcessFrame((uint8_t *)rx_buffer, len);
            }
        }
...
{
    int err = sendto(sock, sendData, dataLen, 0, (struct sockaddr *)&source_addr, sizeof(source_addr));
    if (err < 0) {
        ESP_LOGE(TAG, "Error occurred during sending: errno ");
    }
}
...
static T_ParameterInfo parameterInfo[] = {
        ...
        {2,PAR_VAL_TYPE_FLOAT,"test parameter99", NULL,"test!?"},
    };
